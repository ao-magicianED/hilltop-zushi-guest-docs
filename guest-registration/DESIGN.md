...
### 入力フォームの項目順（スマホで指の移動を最小化）【2026-07-01・v3改訂＝オーナー独自ルール、下記の通り一部差し替え】
1. 区分（代表/同行）→ 2. 氏名（パスポート綴り・英字推奨）→ 3. **日本国内住所の有無（Yes/No）** → 4. 国籍（22種＋その他・よく来る国を上位固定。「その他」選択時のみ自由記入欄をJS表示）→ 5. パスポート画像（**日本国籍以外は全員必須**。国内住所の有無は問わない・旅券番号は収集廃止）→ 6. 住所 → 7. 職業（8種）→ 8. 年齢/性別（**任意**）→ 9. 前泊地/後泊地（**日本国籍は項目自体を非表示。日本国籍以外は前泊地必須・後泊地は任意**）→ 10. 電話番号（**全員任意**）→ 11. メールアドレス（**代表は必須**。同行者は「割引・クーポン情報を受け取る」にチェックした場合のみ必須）→ 12.（代表かつ日本国籍・国内住所ありのみ）今回のご利用用途

> **【レビュー反映 C-3・旧仕様】** 緊急時の連絡手段として同行者にも当日連絡先欄を用意する方針だったが、v3でオーナー判断により電話番号は全員任意に緩和。連絡手段はメール（代表必須）に一本化。
> 旅券写真の要否は「日本国籍か否か」のみで判定（v2までの「日本国籍かつ国内住所あり」からさらに単純化＝国内住所の有無を問わず日本国籍なら不要）。法定（外国籍かつ国内住所なしのみ必須）より広い独自ルールで、法定を下回らない。
...
| 前泊地 | `guests.prev_stay` | 個人 | 【v3】日本国籍以外のみ必須・入力欄表示（日本国籍は非表示＋null化） |
| 後泊地 | `guests.next_stay` | 個人 | 【v3】日本国籍以外のみ表示・任意（日本国籍は非表示＋null化） |
| 職業（8種） | `guests.occupation` | 個人 | コード保存 |
| 国籍（22種＋他） | `guests.nationality` / `nationality_other` | 個人 | OTHER時のみother。自由記入欄はOTHER選択時のみJS表示 |
| 旅券番号 | `guests.passport_no` 🔒 | 個人 | 【v3】収集廃止（列は残置・過去データのみ保持） |
| 電話番号 | `guests.phone` 🔒 | 個人 | 【v3】全員任意（代表必須を撤廃） |
| パスポート画像 | `guests.passport_img_key` 🔒＋mime/size/uploaded_at | 個人 | 実体はKV暗号化保存・10MB検証。【v3】日本国籍以外は全員必須（国内住所の有無は問わない） |
| 年齢 | `guests.age` | 個人 | 【v3】任意 |
| 性別 | `guests.gender` | 個人 | MALE/FEMALE/X。【v3】任意 |
| メールアドレス | `guests.email` | 個人 | 【v3】代表は必須。同行者は割引・クーポン希望チェック時のみ必須 |
| 今回のご利用用途 | `guests.stay_purpose` / `stay_purpose_other` | 個人 | 【v3新設】代表かつ日本国籍・国内住所ありのみ必須（family/friends_family/company/friends/couple/other） |
...
- **「提出済み」の定義（2026-07-01・v3改訂／オーナー独自ルール）**: 全員＝氏名＋住所＋国籍＋職業（年齢・性別・電話は任意）。**「日本国籍」以外**（国内住所の有無は問わない）＝＋旅券写真＋前泊地（旅券番号は求めない・法定より広い独自ルールで法定を下回らない。国内住所ありの外国籍も対象に含める点がv2からの変更）。代表＝＋メール必須（v2まではOTA予約限定だったが常時必須に変更・電話必須は撤廃）。同行者＝割引・クーポン希望チェック時のみメール必須。代表かつ「日本国籍かつ国内住所あり」＝＋利用用途（この設問のみv2と同じ判定条件を維持）。判定はAPI側で行い、満たした瞬間にsubmittedを記録し**そのときの必須ルール版を `submit_rule_version` に保存**（v1=2026-06-29「氏名+住所+職業+国籍+年齢+性別必須＋外国人条件付き旅券番号」、v2=2026-07-01「旅券写真化・年齢性別任意・利用用途新設」として履歴に残る）。
